    ...
    class << self
      ...
      def MakeMask(width, height)
        ...
        for y in 0..(height-1)
          for x in 0..(width-1)
            ...
            c2 = Pixel.new(r*QuantumRange, g*QuantumRange, b*QuantumRange)
            hsl = c2.to_hsla
            if(hsl[2]>0.3*255)
              hsl[2] = ((((hsl[2]/255.0-0.3)/0.35)**0.8)*0.2+0.3)*255
            end
            (0..2).each{ |i|
              hsl[i] = [[0.0, hsl[i]].max, 255.0].min
            }
            c2 = Pixel.from_hsla(hsl[0], hsl[1], hsl[2])
      ...
      def FindMaskColor(mask, i)
        ...
        until success
          ...
          xm += x1
          im = mask.pixel_color(xm, ym).intensity()/(1.0*QuantumRange)
          if((i-im).abs < 0.05)
            success = true
          end
        end
      ...
      def FindGrayColor(i, x, y)
        ...
        if ((x%8<4) ^ (y%8<4))
          r = (0.37+i/3.0)*QuantumRange
          g = (0.62-i*ratio/3.0)*QuantumRange
          b = (0.8-i/3.0)*QuantumRange
        else
          r = (0.44+i/3.0)*QuantumRange
          g = (0.62-i*ratio/3.0)*QuantumRange
          b = (0.9-i/3.0)*QuantumRange
        end
      ...
      def FindDistractionColor(c, i)
        success = false
        until success
          red = (rand()*0.25-0.03)*QuantumRange
          if(c.red+red<0)
            red = -c.red
          end
          if(c.red+red>QuantumRange)
            red = QuantumRange-c.red
          end
          green = (rand()*0.3-0.03)*QuantumRange
          if(c.green+green<0)
            green = -c.green
          end
          if(c.green+green>QuantumRange)
            green = QuantumRange-c.green
          end
          blue = -(red*ALPHA+green*BETA)/(1.0-ALPHA-BETA)
          success = (c.blue+blue>=0 && c.blue+blue<=QuantumRange)
        end
      ...
      def GenerateBWDisappearingImage(message)
        ...
        for y in 0..(height-1)
          for x in 0..(width-1)
            ...
            i = c.intensity()/(1.0*QuantumRange)
      ...
      def GenerateColorDisappearingImage(message, distractionString)
        ...
        for y in 0..(height-1)
          for x in 0..(width-1)
            ...
            di = distraction.pixel_color(x, y).intensity()/(1.0*QuantumRange)
